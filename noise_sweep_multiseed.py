"""
Stage 3 of NOISE_SEARCH_PLAN.md: reduce metric noise on the top ~5 configs from
the stage 1+2 sweeps by training each with 3 seeds and averaging tune/holdout
excess, since a single-seed score can mislead when candidates are this close.
"""
import time
import numpy as np
import pandas as pd

import noise_sweep as ns
...
for cfg in CANDIDATES:
    ...
    rows.append(row)
    pd.DataFrame(rows).to_csv("noise_search_multiseed.csv", index=False)
