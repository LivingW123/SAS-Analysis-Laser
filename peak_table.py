...
Usage
-----
  python peak_table.py
  CNN_NBINS=20 python peak_table.py
  CNN_NBINS=50 python peak_table.py res/test_images/11696 res/test_images/11705 ...
...
The model is loaded by rebuilding train_cnn.build_model(N) and loading weights,
rather than tf.keras.load_model, so that .keras files saved with a newer Keras
(carrying a `quantization_config` Dense field that older releases reject) still
load. Falls back to load_model if the rebuild path is unavailable.
"""
...
def load_cnn(n_bins: int):
    ...
    path = f"{MODEL_PREFIX}_n{n_bins}.keras"
    try:
...
def main() -> None:
    ...
    out_csv = f"cnn_peak_table_n{N_BINS}.csv"
