...
same tool data_utils.py already uses for the saturation-column fit) directly
against the real detector signal with a4 hard-constrained to [10,20] MeV --
the same role PFF.m's classical fit plays, but seeded from the network
instead of a from-scratch initial guess.
...
Usage
-----
  python refine_bump_center_optimizer.py
...
if __name__ == "__main__":
    ...
    pd.DataFrame(rows).to_csv("pff_bumpcenter_refined.csv", index=False)
    print("\nSaved pff_bumpcenter_refined.csv")
