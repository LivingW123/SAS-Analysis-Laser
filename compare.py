"""
StripNet evaluation: test-set metrics + real-b inference.
Outputs: figures/comparison.png
"""
...
stats   = np.load("norm_discrete.npy")     # shape (2, 200): [mean, std] of log1p(X)
...
# ── real-b inference ──────────────────────────────────────────────────────────
b_raw     = np.load("old_experiments/b_real_200.npy")
...
fig.suptitle(
    f"StripNet  |  MAE act={mae_a:.2f}  zero={mae_z:.2f}  recall={recall:.0%}  "
...
xs = np.sort(frl); ys = np.linspace(0, 1, len(xs))
ax_cdf.plot(xs, ys, color="C0", lw=1.5)
...
ax_cdf.set_title("Forward relL2 CDF"); ax_cdf.set_xlim(left=0); ax_cdf.legend(fontsize=7)
...
ax_flux.axvline(np.median(flux_r), color="C1", lw=1.2, ls="--", label=f"med {np.median(flux_r):.2f}")
ax_flux.set_xlabel("Flux ratio  DRM@pred / DRM@true"); ax_flux.set_ylabel("Density")
ax_flux.set_title("Flux ratio distribution"); ax_flux.legend(fontsize=7)
...
for col_i, (pred, true, raw_i) in enumerate(zip(pred6[:2], true6[:2], idx6[:2])):
    ...
    ax.bar(energy_bins, true, color="C0", alpha=0.6, label="True", width=1.0)
    ax.bar(energy_bins, pred, color="C1", alpha=0.6, label="Pred", width=1.0)
...
for i in range(3):
    p, t = pred6[i], true6[i]
    ax_fwd.plot(DRM @ t, color=f"C{i}", lw=1.2, label=f"True {i}")
    ax_fwd.plot(DRM @ p, color=f"C{i}", lw=1.0, ls="--", label=f"Pred {i}")
ax_fwd.set_title("Forward consistency  DRM @ spectrum (3 samples)", fontsize=9)
ax_fwd.set_xlabel("Detector channel"); ax_fwd.set_ylabel("Signal")
ax_fwd.legend(fontsize=6, ncol=2)

ax_real = fig.add_subplot(gs[2, 2])
ax_real.bar(np.arange(200), spec_real, color="C3", alpha=0.8, width=1.0)
...
ax_fwdr.set_xlabel("Detector channel", fontsize=7); ax_fwdr.set_ylabel("Counts", fontsize=7)
ax_fwdr.legend(fontsize=7)

plt.savefig(f"{FIGS_DIR}/comparison.png", dpi=130, bbox_inches="tight")
print(f"Saved {FIGS_DIR}/comparison.png")
