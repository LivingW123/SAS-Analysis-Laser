"""
test_dnn_11733.py — formerly the root-level `infer_11733.py`.

Runs inference on sample 11733 using all trained MeV classifier models
(out/training/dnn/model_dnn_mev_n{n}.keras) and writes
out/testing/inference_11733.csv.
...
OUTPUT_CSV  = os.path.join("out/testing", "inference_11733.csv")
...
def predict_all(signal: np.ndarray, all_results: dict) -> list[dict]:
    ...
    for n in N_VALUES:
        ...
        model_path = os.path.join("out/training/dnn", f"model_dnn_mev_n{n}.keras")
