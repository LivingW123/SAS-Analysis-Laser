...
from src.core.data_utils import bin_drm, generate_synthetic_data, load_drm, mev_bin_centers

XLSX_PATH = "res/drm/200x200.xlsx"
N_VALUES  = [10, 20, 50, 100, 200]
COLORS    = ["steelblue", "darkorange", "forestgreen", "crimson", "purple"]
OUT_DIR   = "out/visualization/figures"
FIGDIR    = OUT_DIR
...
def plot_confusion_matrices() -> None:
    ...
    for ax, n in zip(flat, N_VALUES):
        path = os.path.join("out/training/dnn", f"results_n{n}_confusion.npy")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(json_path):
        print(f"\n{json_path} not found — run train_dnn_mev.py first.")
        sys.exit(1)
