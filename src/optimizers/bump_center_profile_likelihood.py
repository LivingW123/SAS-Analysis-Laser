...
src.core.data_utils.PFF_PARAM_BOUNDS), and at each fixed a4 refits
a1/a2/a3/a5/a6 by bounded least-squares against the real detector signal.
The result is a profile-likelihood curve (residual RMS vs a4) for each real
shot -- an answer to "which bump centre actually explains this shot's
signal best" that doesn't depend on any prior (physical or learned) at all,
only the DRM forward model and the raw channel data. This is the deciding
evidence for whether recentring the training prior on [10,20] MeV is fixing
a real mismatch or just teaching the network a different wrong answer.

Usage
-----
  python -m src.optimizers.bump_center_profile_likelihood
...
-------
  Console: per-shot best-fit a4 (global, unconstrained by any prior) vs
           ...
           for comparison.
  out/optimizers/bump_center_profile.csv : full (shot, a4, residual_rms) grid for plotting.
  out/optimizers/bump_center_profile.png : residual-vs-a4 curves, all bump-present shots.
"""
...
if __name__ == "__main__":
    ...
    fig.savefig(os.path.join(OUT_DIR, "bump_center_profile.png"), dpi=150)

    pd.DataFrame(all_rows).to_csv(os.path.join(OUT_DIR, "bump_center_profile.csv"), index=False)
    summary_df = pd.DataFrame(summary_rows)
    summary_df.to_csv(os.path.join(OUT_DIR, "bump_center_profile_summary.csv"), index=False)
    ...
    print("Saved out/optimizers/bump_center_profile.png, out/optimizers/bump_center_profile.csv, "
          "out/optimizers/bump_center_profile_summary.csv")
