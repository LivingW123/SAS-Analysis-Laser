...
Usage
-----
  python -m src.optimizers.noise_sweep                       # default grid
  python -m src.optimizers.noise_sweep --mode random --trials 40
  python -m src.optimizers.noise_sweep --samples 40000 --epochs 25 --nbins 50
Env: CNN_NBINS also honored. Results -> out/optimizers/noise_search_results.csv
"""
...
from src.core.data_utils import bin_drm, load_drm, generate_spectrum_batch, normalize_fit, normalize_apply, SAT_FRACTION
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(OUT_DIR, "noise_search_results.csv"))
    args = ap.parse_args()
