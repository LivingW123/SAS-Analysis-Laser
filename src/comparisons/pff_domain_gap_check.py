...
Usage
-----
  python -m src.comparisons.pff_domain_gap_check
"""
...
if __name__ == "__main__":
    ...
    real_df.to_csv(os.path.join(OUT_DIR, "pff_domain_gap_real_shots.csv"), index=False)
    synth_df.to_csv(os.path.join(OUT_DIR, "pff_domain_gap_synthetic.csv"), index=False)
    ...
    out_path = os.path.join(OUT_DIR, "pff_domain_gap_check.png")
    fig.savefig(out_path, dpi=150)
    print(f"\nSaved {out_path}")
    print(f"Saved {os.path.join(OUT_DIR, 'pff_domain_gap_real_shots.csv')} and "
          f"{os.path.join(OUT_DIR, 'pff_domain_gap_synthetic.csv')}")
