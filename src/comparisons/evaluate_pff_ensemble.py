...
this project (src.core.real_shots.SHOTS), plus a synthetic batch drawn from
the same generator training uses (known in-distribution by construction),
and reports both epistemic and aleatoric variance separately for both sets.

The real-shot loader / ensemble-loading / ensemble-prediction machinery
(SHOTS, l1_normalise, load_signal, load_members, predict_ensemble) live in
src/core/real_shots.py, shared with the optimizer scripts.

Usage
-----
  python -m src.comparisons.evaluate_pff_ensemble
"""
...
from src.core.data_utils import generate_pff_training_data, load_drm
...
if __name__ == "__main__":
    ...
    combined.to_csv(os.path.join(OUT_DIR, "pff_ensemble_eval.csv"), index=False)
    print(f"\nSaved {os.path.join(OUT_DIR, 'pff_ensemble_eval.csv')}")
    ...
    out_path = os.path.join(OUT_DIR, "pff_ensemble_epistemic_check.png")
