...
Usage
-----
  python -m src.comparisons.nnls_refine        # default shots, n_bins=10, sweeps lambda
  CNN_NBINS=20 python -m src.comparisons.nnls_refine
  CNN_MODEL_PREFIX=model_cnn_denoised CNN_RESULTS_JSON=out/_unsorted/cnn_denoised_training_results.json \
      python -m src.comparisons.nnls_refine   # use the joint-trained model's predictions as x0
      # (model_cnn_denoised is an archived/orphaned model variant, no longer
      # produced by the trainer; its weights + results json now live under
      # out/_unsorted/ rather than out/training/cnn/)
"""
...
# model_cnn_denoised (the joint-trained variant, selected via CNN_MODEL_PREFIX
# below) is an archived/orphaned artifact no longer produced by the trainer;
# its weights + results json live under out/_unsorted/ instead.
ARCHIVED_MODEL_DIR = "out/_unsorted"
...
def get_cnn_spec(signal_l1: np.ndarray, drm_b: np.ndarray) -> np.ndarray:
    if IS_JOINT:
        # archived/orphaned joint-trained variant -- see ARCHIVED_MODEL_DIR note above
        model = tf.keras.models.load_model(
            os.path.join(ARCHIVED_MODEL_DIR, f"{MODEL_PREFIX}_n{N_BINS}.keras"), compile=False)
    ...
    model = tf.keras.models.load_model(os.path.join(MODEL_DIR, f"{MODEL_PREFIX}_n{N_BINS}.keras"), compile=False)
...
def main() -> None:
    ...
    out_csv = os.path.join(OUT_DIR, f"nnls_hybrid_eval_n{N_BINS}{'_denoised' if IS_JOINT else ''}.csv")
