...
exits. State persists in out/training/cnn/cnn_chunk_state.json so repeated
invocations continue where the last one stopped. Designed for environments
with a hard per-process wall-time cap; on a normal machine just use
train_cnn.py.

Usage:  python -m src.training.cnn.train_cnn_chunk [train_seconds] [n_bins]
"""
