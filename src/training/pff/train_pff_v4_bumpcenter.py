...
Usage
-----
  python -m src.training.pff.train_pff_v4_bumpcenter

Outputs
-------
  out/training/pff/model_pff_v4_bumpcenter.keras
  out/training/pff/pff_training_results_v4_bumpcenter.json
  out/training/pff/pff_v4_bumpcenter_real_shots.csv  (a4 mean/sigma on the
      same 14 real shots used throughout this session)
"""
...
MODEL_PATH        = os.path.join(OUT_DIR, "model_pff_v4_bumpcenter.keras")
RESULTS_JSON       = os.path.join(OUT_DIR, "pff_training_results_v4_bumpcenter.json")
REAL_SHOTS_CSV     = os.path.join(OUT_DIR, "pff_v4_bumpcenter_real_shots.csv")
...
if __name__ == "__main__":
    ...
    pd.DataFrame(rows).to_csv(REAL_SHOTS_CSV, index=False)
    print(f"Saved {REAL_SHOTS_CSV}")
