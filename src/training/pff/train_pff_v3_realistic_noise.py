...
classifier (src/training/cnn/train_cnn.py) already uses a fully realistic,
saturation-aware generator (data_utils.generate_spectrum_batch, calibrated via
a staged search against real shots' NNLS floor -- see src/optimizers/noise_sweep.py
and noise_sweep_multiseed.py; the methodology writeup this once referenced,
VALUE_SUMMARY.md, no longer exists in the repo). This
...
Usage
-----
  python -m src.training.pff.train_pff_v3_realistic_noise

Outputs
-------
  out/training/pff/model_pff_v3.keras
  out/training/pff/pff_training_results_v3.json
"""
...
# Own output basename (model_pff_v3.keras), now relocated under OUT_DIR
# alongside every other PFF trainer's outputs.
MODEL_PATH   = os.path.join(OUT_DIR, "model_pff_v3.keras")
RESULTS_JSON = os.path.join(OUT_DIR, "pff_training_results_v3.json")
