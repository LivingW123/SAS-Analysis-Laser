...
Usage
-----
  python -m src.training.pff.train_pff_ensemble_member --idx 0
  python -m src.training.pff.train_pff_ensemble_member --idx 1
  ... --idx 2, 3, 4

Outputs (per member idx)
-------
  out/training/pff/model_pff_ensemble_{idx}.keras
  out/training/pff/pff_training_results_ensemble_{idx}.json
"""
...
if __name__ == "__main__":
    ...
    MODEL_PATH   = os.path.join(OUT_DIR, f"model_pff_ensemble_{idx}.keras")
    RESULTS_JSON = os.path.join(OUT_DIR, f"pff_training_results_ensemble_{idx}.json")
