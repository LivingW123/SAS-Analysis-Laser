...
exponential + one Gaussian bump (src.core.data_utils.generate_spectrum_batch /
...
def train_for_n(drm: np.ndarray, n_bins: int, rng: np.random.Generator) -> tuple[dict, tf.keras.Model]:
    ...
    model_path = os.path.join(OUT_DIR, f"model_dnn_spectrum_n{n_bins}.keras")
    callbacks = [
...
if __name__ == "__main__":
    ...
    json_path = os.path.join(OUT_DIR, "dnn_spectrum_training_results.json")
    all_results: dict = {}
