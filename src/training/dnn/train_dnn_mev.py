...
def train_for_n(
    ...
    np.save(os.path.join(OUT_DIR, f"results_n{n}_confusion.npy"), cm)
...
def save_csv_results(all_results: dict) -> None:
    ...
    for res in all_results.values():
        n = res["n"]
        epoch_path = os.path.join(OUT_DIR, f"results_epochs_n{n}.csv")
...
if __name__ == "__main__":
    ...
    json_path = os.path.join(OUT_DIR, "dnn_mev_training_results.json")
    all_results: dict = {}
    ...
    for n in N_VALUES:
        model_path = os.path.join(OUT_DIR, f"model_dnn_mev_n{n}.keras")
