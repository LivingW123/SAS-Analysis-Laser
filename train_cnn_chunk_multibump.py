"""
Resumable chunked trainer for the 5x48 CNN, multibump variant.

Identical to train_cnn_chunk.py except training data is drawn from
data_utils.generate_multibump_spectrum_batch (0..MAX_BUMPS independent
Gaussian bumps) instead of the single-bump PFF form, so the model can
learn spectra with more than one spectral feature — e.g. the unmodeled
detector-channel 100-150 bump seen in shots 10084 / 11733 with the
single-bump models.

State/checkpoints are kept separate from the single-bump run (distinct
file names) so neither overwrites the other.

Usage:  python train_cnn_chunk_multibump.py [train_seconds] [n_bins]
"""
...
from data_utils import bin_drm, generate_multibump_spectrum_batch, load_drm, normalize_apply, normalize_fit
...
STATE_PATH = f"cnn_chunk_state_multibump_n{N_BINS}.json"
CKPT_PATH  = f"model_cnn_multibump_n{N_BINS}_ckpt.keras"
BEST_PATH  = f"model_cnn_multibump_n{N_BINS}.keras"
...
def main() -> None:
    ...
    if os.path.exists(STATE_PATH):
        ...
        X_boot, _ = generate_multibump_spectrum_batch(drm_binned, 10_000, rng0, BUMP_FRACTION)
    ...
    X_val, y_val = generate_multibump_spectrum_batch(drm_binned, VAL_SAMPLES, val_rng, BUMP_FRACTION)
    ...
    while time.perf_counter() - t_budget_start < TRAIN_SECONDS:
        Xb, yb = generate_multibump_spectrum_batch(drm_binned, slice_samples, chunk_rng, BUMP_FRACTION)
